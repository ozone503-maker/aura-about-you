...
const choices = [
  ...
  { tone: "indigo", title: "Hypnosis", detail: "Consultation before booking", copy: "Begin with a conversation about your goal, questions, and whether the work is a good fit.", subject: "Hypnosis consultation" },
  { tone: "red", title: "Energy clearing", detail: "Personal or property · quote by request", copy: "Tell Shana what is happening, where you are located, and what kind of support you are seeking.", subject: "Energy clearing inquiry" },
  { tone: "orange", title: "Private event", detail: "Custom proposal", copy: "Include your date, city, guest count, event length, and the experience you have in mind.", subject: "Private event inquiry" },
] as const;
  ...
  return <main className="tone-violet"><Header />
    ...
    <section className="bookingChoices" aria-label="Session choices">
      {choices.map((choice) => <article className={`bookingChoice booking-${choice.tone}`} key={choice.title}>
        <p className="eyebrow">{choice.detail}</p><h2>{choice.title}</h2><p>{choice.copy}</p>
    ...
    <section className="bookingNote"><p>Sessions support personal exploration and relaxation. They do not diagnose, treat, or replace medical or mental-health care.</p><Link href="/aura-photography">Learn how aura photography works →</Link></section>
    <Footer />
  </main>;
}
